...
const createCustomButton = () => {
  ...
  button.className = 'jobs-save-button artdeco-button artdeco-button--secondary artdeco-button--3 custom-button';
  button.type = 'button';
  ...
  const observer = new MutationObserver((mutations) => {
    for (const mutation of mutations) {
      if (mutation.type === 'childList') {
        const container = document.querySelector('.jobs-apply-button--top-card')?.closest('.display-flex');
        if (container) {
